...
- 目录仍是保持相同顺序的可搜索扁平列表。为便于扫描，目的地分为四个带标题
  ...
  MCP、子智能体）、“工作区”（导入、项目）、`数据与统计`（索引）和“系统”（信息）。标题使用柔和
...
### 索引库（`index` 选项卡，`数据与统计` 分组）
- 单张健康卡展示 host 所有的工作区索引缓存：状态、已索引文件数、已索引体积、
  无法读取的文件数、最近更新时间
- 第二个开关「索引新文件夹」（`indexNewFolders`，默认关）：开启后，切换到不同
  工作区会把其索引标记为 `building` 并在阻塞线程池中重建，`workspace.set` 保持
  快速；健康卡在构建期间每秒轮询一次 `index.status`，窗口隐藏时暂停
- 单个开关「索引加速 Grep」（`indexGrepBoost`，默认关）：开启后，区分大小写的
  字面量 Grep 查询可以由索引服务；其余查询照常全量走查。文案不承诺结果变化，
  只说明符合条件的查询会更快。
- 操作为「重建索引」与「清理索引」。重建把当前工作区扫描进可丢弃缓存；清理删除
  其 root namespace。两者都调用 host 生命周期 RPC 并用返回状态刷新卡片
- 文案说明索引是可重建的本地缓存，数据不会离开本机。不得声称 Grep 读取该索引：
  字面量快路径将单独发布，在此之前 Grep 行为保持不变
- 空态：当前工作区还没有 root，只提供「建立索引」动作。加载失败显示重试而不是空白卡片
