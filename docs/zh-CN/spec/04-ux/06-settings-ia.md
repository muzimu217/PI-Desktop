...
修订 D335 / ADR 0173（ADR 0270）：Usage 目的地回归设置页，提供宿主自有口径。
...
- 单张健康卡展示 host 所有的工作区索引缓存：状态、已索引文件数、已索引体积、
  无法读取的文件数、最近更新时间
- 单个开关「索引加速 Grep」（`indexGrepBoost`，默认关）同时负责索引的生产与消费，
  因为 Grep 是它唯一的消费方。开启后，切换到不同工作区会把其索引标记为 `building`
  并在阻塞线程池中重建，`workspace.set` 保持快速；健康卡在构建期间每秒轮询一次
  `index.status`，窗口隐藏时暂停；区分大小写的字面量 Grep 查询此时可由索引服务，
  其余查询照常全量走查。文案不承诺结果变化，只说明符合条件的查询会更快。再加一个
  「索引新文件夹」开关要么与它重复，要么建出无人读取的索引，因此卡片上只保留一个
