...
> **套件说明（2026-09-16）**：在 D335 / ADR 0173 继续有效期间，设置中不提供「使用统计」
> 目的地，因此 `pnpm test:e2e:stats` 会报告 SKIPPED 并以 0 退出，而不是对着当时恰好激活的
> 选项卡做断言。下列旅程仍是「插件重新路由该看板」后的计划；在此期间 host-core
> `stats::tests` 是可复现的底线。
