...
D335 / ADR 0173 继续有效，ADR 0271 已被取代：不提供 `usage` 目的地，也不为其
保留分组。宿主自有的聚合口径（`stats.summary` / `stats.topSessions`，基于持久
的已完成回合历史）与 `stats.getTokenUsageHistory` 一同留在 Core 表面，
`components/settings/StatsPage.tsx` 作为它的参考消费方保留。跨工具仪表盘仍由
`pi.token-insights` 插件承担。
